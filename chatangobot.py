...
connection = sqlite3.connect('fresh.sqlite')
...
class bot(ch_fixed.RoomManager):
  ...
  def onPMMessage(self, pm, user, response):
    ...
    if row is None:
        ...
        row = cursor.fetchone()
    #if there is an empty database, do this instead
    if row is None:
        # repeat what was messaged instead
        B = H
    else:
    ...
    self.safePrint('Reply: ' + username + ': ' + B)
    pm.message(user, B) # response
